...
if (isset($_GET['lang']) && in_array($_GET['lang'], ['id', 'en', 'cn'])) {
    ...
    // Bangun ulang URL (agar parameter seperti ?action=edit_profile tidak hilang)
...
// 8. Fungsi Helper Format Tanggal Manual (Agar Konsisten dengan Student Profile)
...
$teacher_id = $user['id']; // ID dari session (Aman)
$success_message = '';
$error_message = '';

// [BARU] Cek apakah ada pesan sukses dari redirect sebelumnya (Flash Message)
if (isset($_SESSION['flash_success'])) {
    $success_message = $_SESSION['flash_success'];
    unset($_SESSION['flash_success']); // Hapus agar tidak muncul terus
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    
    // ... (Logika update_profile TETAP SAMA, jangan diubah) ...
    // --- 1. HANDLE UPDATE PROFIL (DATA DIRI) ---
    if (isset($_POST['action']) && $_POST['action'] === 'update_profile') {
        try {
            ...
            $phone_number = substr(trim($raw_phone), 0, 16);
            ...
            // 2. Redirect/Pindah ke halaman 'Lihat Data Diri' (view_data)
            ob_end_clean();
            header("Location: teacher-dashboard.php?page=profile&action=view_data");
            exit(); 
            // ------------------------------------
    ...
    // --- 2. HANDLE GANTI KATA SANDI (UBAH BAGIAN INI) ---
            ...
            if ($new_password === $confirm_password) {
                if (strlen($new_password) >= 6 && strlen($new_password) <= 50) {
                    try {
                        ...
                        // 2. Redirect ke halaman profil utama
                        ob_end_clean(); // Bersihkan output buffer sebelum redirect
                        header("Location: teacher-dashboard.php?page=profile");
                        exit();
...
<?php if ($action == 'view_data'): ?>
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="teacher-dashboard.php?page=profile" class="text-sm text-orange-600 hover:text-orange-700 flex items-center gap-1">
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg shadow-orange-900/5">
        <div class="flex justify-end mb-6">
             <a href="teacher-dashboard.php?page=profile&action=edit_profile" class="btn-gradient text-white font-semibold px-6 py-2 rounded-lg inline-flex items-center gap-2">
        ...
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_name'); ?></dt>
            ...
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_phone'); ?></dt>
                <dd class="text-gray-800 mt-1"><?php echo htmlspecialchars($user['phone_number'] ?: '-'); ?></dd>
            </div>
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_birth_place'); ?> & <?php echo __('teacher_form_label_birth_date'); ?></dt>
            ...
            <div class="border-b border-gray-100 py-3 md:col-span-2">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_address'); ?></dt>
                <dd class="text-gray-800 mt-1"><?php echo htmlspecialchars($user['address'] ?: '-'); ?></dd>
            </div>
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_employment_status'); ?></dt>
                <dd class="text-gray-800 mt-1"><?php echo ($user["employment_status"] === 'full-time') ? __('teacher_form_status_full_time') : __('teacher_form_status_part_time'); ?></dd>
            </div>
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_subjects'); ?></dt>
                <dd class="text-gray-800 mt-1"><?php echo htmlspecialchars($user['subjects'] ?: '-'); ?></dd>
            </div>
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_days'); ?></dt>
                <dd class="text-gray-800 mt-1"><?php echo htmlspecialchars($user['available_days'] ?: '-'); ?></dd>
            </div>
            <div class="border-b border-gray-100 py-3">
                <dt class="font-semibold text-gray-600 text-sm"><?php echo __('teacher_form_label_times'); ?></dt>
...
<?php elseif ($action == 'edit_profile'): ?>
    <div class="flex justify-between items-center mb-6">
        ...
        <a href="teacher-dashboard.php?page=profile&action=view_data" class="text-sm text-orange-600 hover:text-orange-700 flex items-center gap-1">
            <i data-lucide="arrow-left" class="w-4 h-4"></i> <?php echo __('user_form_back_link'); ?>
        </a>
    </div>
    
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg shadow-orange-900/5">
        <form action="teacher-dashboard.php?page=profile&action=edit_profile" method="POST" class="space-y-6">
            <input type="hidden" name="action" value="update_profile">
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1"><?php echo __('teacher_form_label_name'); ?></label>
                ...
                <div>
                    <label for="phone_number" class="block text-sm font-medium text-gray-700 mb-1"><?php echo __('teacher_form_label_phone'); ?></label>
                    <input type="tel" 
                           name="phone_number" 
                           id="phone_number" 
                           class="contact-input" 
                 ...
                 <div class="md:col-span-2">
                    <label for="address" class="block text-sm font-medium text-gray-700 mb-1"><?php echo __('teacher_form_label_address'); ?></label>
                 ...
                 <div>
                    <label for="birth_place" class="block text-sm font-medium text-gray-700 mb-1"><?php echo __('teacher_form_label_birth_place'); ?></label>
                 ...
                 <div>
                    <label for="birth_date" class="block text-sm font-medium text-gray-700 mb-1"><?php echo __('teacher_form_label_birth_date'); ?></label>
            ...
            <p class="text-sm text-gray-500 mt-2"><?php echo __('teacher_profile_note'); ?></p>
            
            <div class="mt-8 flex justify-end gap-4">
                <a href="teacher-dashboard.php?page=profile&action=view_data" class="px-6 py-2 rounded-lg text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors"><?php echo __('teacher_form_button_cancel'); ?></a>
...
<?php elseif ($action == 'edit_password'): ?>
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg shadow-orange-900/5">
        <form action="teacher-dashboard.php?page=profile&action=edit_password" method="POST" id="passwordForm" class="space-y-6 max-w-lg mx-auto">
            ...
            <div class="mt-8 flex justify-end gap-4">
                <a href="teacher-dashboard.php?page=profile" class="px-6 py-2 rounded-lg text-gray-600 bg-gray-100 hover:bg-gray-200 transition-colors"><?php echo __('teacher_form_button_cancel'); ?></a>
    ...
    passwordInputs.forEach(input => {
        if (input) {
            input.addEventListener('input', function() {
                // Hapus spasi secara otomatis jika ada
                if (this.value.includes(' ')) {
                    this.value = this.value.replace(/\s/g, '');
                }
                
                // Hapus pesan error manual (jika ada sisa dari validasi browser)
                const warningId = 'warning-' + this.id.split('_')[0]; // ex: warning-current
...
<?php else: // Default 'main' action ?>
    <h1 class="text-3xl font-bold text-gray-800 mb-6"><?php echo __('profile_page_title'); ?></h1>
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg shadow-orange-900/5">
        <div class="grid md:grid-cols-3 gap-6">
            <div class="md:col-span-1 flex flex-col items-center text-center">
                <div class="w-24 h-24 rounded-full bg-orange-100 flex items-center justify-center mb-4">
                    <i data-lucide="user" class="w-12 h-12 text-orange-500"></i>
                </div>
                ...
                <span class="mt-2 inline-block bg-orange-100 text-orange-700 text-xs font-semibold px-3 py-1 rounded-full capitalize">
                    <?php echo __('profile_role_teacher'); ?>
                </span>
            </div>
            ...
            <div class="md:col-span-2 space-y-6">
                <div>
                    ...
                    <div class="space-y-4">
                        <a href="teacher-dashboard.php?page=profile&action=view_data" class="flex items-center justify-between w-full p-4 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors">
                        ...
                        <a href="teacher-dashboard.php?page=profile&action=edit_password" class="flex items-center justify-between w-full p-4 bg-gray-50 hover:bg-gray-100 rounded-lg transition-colors">
                            <div class="flex items-center gap-4">
